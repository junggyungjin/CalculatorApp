...
import practice.aop.calculatorapp.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    fun buttonClicked(v: View) {
        when(v.id) {
            ...
            R.id.buttonMultil -> operatorButtonClicked("*")
            R.id.buttonDivider -> operatorButtonClicked("/")
            R.id.buttonModullo -> operatorButtonClicked("%")
        }
    }
    ...
    private fun numberButtonClicked(number: String) {
        ...
        if(expressionText.isNotEmpty() && expressionText.last().length >= 15) {
            Toast.makeText(this@MainActivity, "15자리까지만 사용할 수 있습니다", Toast.LENGTH_SHORT).show()
            return
    ...
    private fun operatorButtonClicked(operator: String) {
        ...
        when {
            //연산자를 쓰고 나서 수정을 위해 뒤로가기를 누르지 않고 연산자를 누른 경우에 수정 허용을 위해
            isOperator -> {
                val text = expressionTextView.text.toString()
                expressionTextView.text = text.dropLast(1) + operator // 문자열의 마지막 1개의 제외하고 가져와서 오퍼레이터를 붙인다
            }
            //연산자를 썼는데 또 쓰는 경우
            hasOperator -> {
